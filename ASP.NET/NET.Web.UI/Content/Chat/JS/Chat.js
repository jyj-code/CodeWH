...
; !function (win, undefined) {
    ...
    chatCore.popchatbox = function (othis) {
        var node = chatCore.node, userId = othis.attr('id'), dataId = othis.attr('data-id'), param = {
            ...
            href: 'http://kebue.com/'//config.hosts + 'user/' + dataId //用户主页
    ...
    chatCore.event = function () {
        ...
        systemHub.client.onConnected = function (id, userName, allUsers) {
            ...
            if (onlinenum > 0) {
                ...
                for (; i < onlinenum; i++) {
                    str += '<li id="userid-' + allUsers[i].UserID + '" data-id="' + allUsers[i].ConnectionId + '" class="ChatCore_childnode" type="one"><img src="' + userFaceImage + '?' + allUsers[i].UserID + '"  class="ChatCore_oneface"><span  class="ChatCore_onename">' + allUsers[i].UserName + '(' + ')</span><em class="ChatCore_time">' + allUsers[i].LoginTime + '</em></li>';
                }
                str += '</ul></li>';
                myf.html(str);
            } else {
        ...
        systemHub.client.onNewUserConnected = function (id, userId, userName, loginTime) {
            ...
            str += '<li id="userid-' + userId + '" data-id="' + id + '" class="ChatCore_childnode" type="one"><img src="' + userFaceImage + '?' + userId + '"  class="ChatCore_oneface"><span  class="ChatCore_onename">' + userName + '(' + ')</span><em class="ChatCore_time">' + loginTime + '</em></li>';
            myf.append(str);
        };
